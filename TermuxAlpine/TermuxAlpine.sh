...
seturl() {
	ALPINE_VER=$(curl -s https://mirrors.tuna.tsinghua.edu.cn/alpine/latest-stable/releases/$SETARCH/latest-releases.yaml | grep -m 1 -o version.* | sed -e 's/[^0-9.]*//g' -e 's/-$//')
	if [ -z "$ALPINE_VER" ] ; then
		exit 1
	fi
	ALPINE_URL="https://mirrors.tuna.tsinghua.edu.cn/alpine/latest-stable/releases/$SETARCH/alpine-minirootfs-$ALPINE_VER-$SETARCH.tar.gz"
}
...
finalwork() {
	[ ! -e ${DESTINATION}/finaltouchup.sh ] && curl --silent -LO https://ghproxy.net/https://raw.githubusercontent.com/releee/releee.github.io/main/TermuxAlpine/finaltouchup.sh
	if [ "${MOTD}" = "ON" ]; then
